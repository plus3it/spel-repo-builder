...
This site might be more direct (they change the location constantly, so no guarantees):

* <https://www.cyber.mil/pki-pke/for-admins-integrators-and-developers>
...
2.  Identify the `*DoD.der.p7b` file in the bundle (or similar, the filename changes
    most every the time)
3.  Run this command to export the CA certificates to a concatenated PEM file:

    ```
    openssl pkcs7 -in *DoD.der.p7b -inform der -print_certs -out DoD_CAs.pem
    ```
